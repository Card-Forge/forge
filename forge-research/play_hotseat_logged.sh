#!/bin/bash
# Launch a hotseat game with RL action logging enabled.
# Every decision is recorded in the same format the RL agent sees.
# Log is saved to plays/gui_game_<timestamp>.json on exit.
#
# Usage: ./play_hotseat_logged.sh [deck_a.dck] [deck_b.dck]
#
...
java -cp "$CP" forge.research.PlayHotseat --logged "$DECK_A" "$DECK_B"
