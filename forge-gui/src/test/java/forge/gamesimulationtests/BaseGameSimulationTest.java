...
public class BaseGameSimulationTest extends PowerMockTestCase {
	...
	protected void initializeMocks() throws Exception {
		...
		FModel mockModel = PowerMockito.mock( FModel.class );
		ForgePreferences forgePreferences = new ForgePreferences();
		PowerMockito.when( mockModel, "getPreferences" ).thenReturn( forgePreferences );
		PowerMockito.mockStatic( Singletons.class );
		PowerMockito.when( Singletons.getModel() ).thenReturn( mockModel );
		PowerMockito.when( Singletons.getMagicDb() ).thenReturn( CardDatabaseHelper.getStaticDataToPopulateOtherMocks() );
	}
