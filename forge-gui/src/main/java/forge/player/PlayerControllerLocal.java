...
public class PlayerControllerLocal extends PlayerControllerHuman implements IGameView {
    ...
    public boolean isUiSetToSkipPhase(final Player turn, final PhaseType phase) {
        return !getGui().stopAtPhase(getPlayerView(turn), phase);
    }
    ...
    public Map<Card, Integer> assignCombatDamage(final Card attacker,
        ...
        if (defender != null && assignDamageAsIfNotBlocked(attacker)) {
            ...
            final List<CardView> vBlockers = getCardViews(blockers);
    ...
    public List<Card> chooseCardsForEffect(List<Card> sourceList, SpellAbility sa, String title, int min, int max, boolean isOptional) {
        ...
        getGui().setPanelSelection(getCardView(sa.getHostCard()));
        ...
        return SGuiChoose.many(getGui(), title, "Chosen Cards", min, max, sourceList, getCardView(sa.getHostCard()));
    }
        ...
        if (mayAutoPass()) {
            ...
            if (!mandatoryAttackers.isEmpty()) {
                ...
                for (Pair<Card, GameEntity> attacker : mandatoryAttackers) {
                    ...
                    getGui().fireEvent(new UiEventAttackerDeclared(getCardView(attacker.getLeft()), getGameEntityView(attacker.getRight())));
                }
                try {
    ...
    private final BiMap<SpellAbility, SpellAbilityView> spabs
        = HashBiMap.create();
    ...
    public boolean canUndoLastAction() {
        ...
        if (priorityPlayer == null || priorityPlayer != this.player) {
            return false;
        }
        return true;
    }
    ...
    private List<StackItemView> getStack(final Iterable<SpellAbilityStackInstance> stack) {
        synchronized (this) {
            stackItems.keySet().retainAll(Lists.newArrayList(stack));
    ...
    public SpellAbilityStackInstance getStackItem(final StackItemView view) {
        return stackItems.inverse().get(view);
    }
    ...
    public void devGenerateMana() {
        DevModeUtil.devModeGenerateMana(game, this);
    }
    @Override
    public void devSetupGameState() {
        DevModeUtil.devSetupGameState(game, this);
    }
    @Override
    public void devTutorForCard() {
        DevModeUtil.devModeTutor(game, this);
    }
    @Override
    public void devAddCardToHand() {
        DevModeUtil.devModeCardToHand(game, this);
    }
    @Override
    public void devAddCounterToPermanent() {
        DevModeUtil.devModeAddCounter(game, this);
    }
    @Override
    public void devTapPermanent() {
        DevModeUtil.devModeTapPerm(game, this);
    }
    @Override
    public void devUntapPermanent() {
        DevModeUtil.devModeUntapPerm(game, this);
    }
    @Override
    public void devSetPlayerLife() {
        DevModeUtil.devModeSetLife(game, this);
    }
    @Override
    public void devWinGame() {
        DevModeUtil.devModeWinGame(game, this);
    }
    @Override
    public void devAddCardToBattlefield() {
        DevModeUtil.devModeCardToBattlefield(game, this);
    }
    @Override
