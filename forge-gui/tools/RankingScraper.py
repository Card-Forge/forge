...
def manualRankings(edition='CMR'):
...
def draftsimRankings(edition='CMR', extra=None):
    edition = edition.upper()
    url = 'https://draftsim.com/generated/%s.js' % (edition)
    r = requests.get(url)
    tx = r.text
    start = tx.find('[')
    end = tx.rfind(']')
    ...
    txt2 = ""
    if extra:
        urlx = 'https://draftsim.com/generated/%s.js' % extra
        x = requests.get(urlx)
        tx2 = x.text
        start = tx2.find('[')
        end = tx2.rfind(']')
...
if __name__ == "__main__":
    ...
    parser.add_argument('-m', action='store_true', dest='manual', help='Use manual rankings', required=False)
