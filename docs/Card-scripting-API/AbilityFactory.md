...
- `ValidTgts$ {Valid}` if the ability targets
...
The SpellDescription for secondary abilities is displayed when (and if) the ability prompts for user input in the prompt pane so it is useful to put some text there or rather split the description into the different effect parts (aligning it with the linebreaks from the Oracle also helps with card translations).
...
A good practice against information overload (and engine overhead!) from effects is cleaning them up (even in case their duration is technically not reached yet) as soon as they can no longer cause any changes.
This follows our general approach where we try to find a reasonable middle ground between parts the scripter has to take care of and the engine helping with auto-detecting the more common cases so it's not burdened too much. 
