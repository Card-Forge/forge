...
| 11d | No detection that game state is *repeating* — the reliable loop signal | `GameStateFingerprint` hashed after each resolution; 3 consecutive identical fingerprints = loop — **DONE** — `GameStateFingerprint.compute()` over player life/poison/commander-damage/mana + per-zone name multiset + in-turn position; `Game.recordLoopState()` called from `MagicStack.resolveStack()` after each resolution, declares draw at 3 repeats (`GameStateFingerprint.java`, `Game.java`, `MagicStack.java:638`) | #48 | v1 |
| 11e | No player-facing flow when a loop is found | Loop-detected dialog: Declare Draw / Break Loop / Continue (max 5); mandatory loops skip dialog → immediate draw; AI breaks or draws autonomously | **PARTIAL** — #48 fingerprint detects the loop and prompts via the existing `DeclareLoop` confirm flow: human gets Declare Draw / Continue (capped at 5 continuations via `loopContinuationBudget`), AI draws autonomously; "Break Loop" alternative-action and mandatory-loop detection are not implemented (`Game.recordLoopState(SpellAbility)`, `MagicStack.resolveStack()` prompt block, `Game.java`/`MagicStack.java`) | #49 | v1 |
| 11f | Deterministic loops (e.g. Umbral Mantle) require manual iteration with full UI cost | `RepeatNExecutor`: "Repeat N times" executed server-side with fingerprint verification + Esc interrupt; AI computes outcome mathematically | **PARTIAL** — engine core landed: `RepeatNExecutor` (new `forge.game.repeat` package) executes a caller-supplied ability N times via `GameStateFingerprint` determinism gate (fingerprint before each iteration must match the previous; divergence aborts) and honors an `interrupt` flag; `canRepeatDeterministically(SpellAbility)` rejects triggered/targeted abilities. UI deferred: rendering the count dialog + 10/50/100/1000 presets, capturing the player's activation as a fresh `SpellAbility` per iteration (auto-paying cost + auto-resolving choices inside the factory), and driving `interrupt` from Esc/Stop — see issue #50. AI mathematical outcome is out of scope for v1 (`RepeatNExecutor.java`) | #50 | v2 |
| 11g | Repetitive triggers require manual response each time | Auto-yield system (MTGO-style): yield this trigger / this turn / all from card / all of type; "Manage Yields" panel | **DONE** — already implemented upstream: `AutoYieldStore` (Tier/TriggerDecision, `onGameEnd` clears GAME tier), `PlayerControllerHuman.confirmTrigger()` consults `getTriggerDecision(wrapper.yieldKey())` before prompting, and `YieldController` (shouldAutoYield/setShouldAutoYield/getAutoYields/clearAutoYields/autoPassUntilEndOfTurn/abilityScope) covers every spec category (always-yield-this-trigger, yield-this-turn, yield-all-[card], yield-[trigger-type]); closed #51 as already-implemented | #51 | v2 |
| 11h | No digital equivalent of paper shortcut proposal (CR 720) | `ShortcutManager`: declare loop + count → opponents accept/lower/interrupt/object; mandatory loop objection → draw | **PARTIAL** — engine core landed: new `forge.game.shortcut` package with `ShortcutDeclaration` (controller + loop abilities + proposed count + per-opponent response map + status machine DECLARED→ACCEPTED/LOWERED/INTERRUPTED/OBJECTED→EXECUTING→COMPLETE) and `ShortcutManager` (one in-flight declaration; `declareShortcut`/`accept`/`lower`/`interrupt`/`object`; `object` on a mandatory loop calls `Game.declareLoopDraw()`; `beginExecution` runs the loop via `RepeatNExecutor` #50 with a game-over guard). UI deferred: the loop-builder dialog (count entry + 10/50/100/1000 presets + capturing the player's activation as a fresh `SpellAbility` per iteration), the opponent-response prompts (accept/lower/interrupt/object) driven by `PlayerActionConfirmMode`, and multiplayer response sync — see issue #52. `Game` holds a `ShortcutManager` field with `getShortcutManager()` (`Game.java` ~282, 643) | #52 | v3 |
...
| 12f | Visual polish — no hover glow, no drop shadows, no rounded panels | Dated look despite FlatLaf theme | Card hover glow animation, rounded corners on panels, drop shadows on zoom, phase strip gradient | open |
...
| P2 | 12f — Visual polish (hover glow, drop shadows, rounded panels) (#88) | 1-2 days | Modern feel; shared painting utilities |
