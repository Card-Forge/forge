...
- **Auto-Pass** — a persistent toggle that automatically yields priority when you have no playable actions. Available on **Desktop** via the Auto-Pass dock icon or F2 hoktey, or on **Mobile** from the in-match Game menu.
...
By default, every yield except **Resolve entire stack** cancels automatically when an opponent casts a spell or declares attackers against you, so you can respond or block. You can also press Escape or click Cancel at any time to break out of a yield.
