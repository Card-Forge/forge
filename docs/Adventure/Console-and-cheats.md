...
| fly &lt;duration&gt; | Allows you to walk over obstacles in the overworld or POI map for a number of seconds (TODO BUG: useless in POIs) |
| give boosters &lt;edition code&gt; \[amount\] | Adds a booster of a set (Beta is leb) |
...
| give print &lt;edition code&gt; &lt;collector number&gt; | Adds a specific printing of a card to your inventory (Black Lotus from Alpha is lea 232) |
| give quest &lt;id&gt; | Adds the quest by its number id |
| give set &lt;edition code&gt; | Adds 4 copies of every card of a set to your inventory (Secret Lair Drop is SLD), flagged as having no sell value |
...
| hide &lt;duration&gt; | Makes enemies not chase you in the overworld or POI map for a number of seconds (TODO BUG: useless in POIs) |
| leave | Gets you out of the current POI map (town/dungeon/cave) and back to the overworld |
...
| reset map | Resets the current POI map (not overworld) after you leave it, restoring all deteted POI map objects like enemies |
...
| dumpEnemyColorIdentity | Prints all enemies, with their colour affinity and deck name, to stdout (TODO: useless on mobile and makes the app unresponsive) |
| dumpEnemyDeckColors | Prints all decks available to enemies and their affinities to stdout (TODO: useless on mobile and makes the app unresponsive) |
| dumpEnemyDeckList | Prints the enemy deck lists to stdout (TODO: useless on mobile and makes the app unresponsive) |
| listPOI | Prints all locations, as ID-type pairs, to stdout (TODO: useless on mobile) |
| fullHeal | Same as heal full command (TODO: redundant) |
| getShards amount &lt;amount&gt; | Same as give shards command (TODO: redundant) |
