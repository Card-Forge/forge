...
The reason is the card corpus. `forge-gui/res/cardsfolder/` holds **tens of thousands of card scripts**, and upstream
...
a conflict on the next sync, forever. Upstream is over half a million lines of Java across some 2,600 files and 12 Maven
modules, plus 61 Markdown files under `docs/`, plus its TestNG suite. Nothing on the Crucible side needs to modify any
of it.
...
**Bad.** The repository carries half a million lines of Java that Crucible never executes, which makes clones large and
