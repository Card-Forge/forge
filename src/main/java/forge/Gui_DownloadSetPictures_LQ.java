...
public class Gui_DownloadSetPictures_LQ extends GuiDownloader {
     ...
     */
    public Gui_DownloadSetPictures_LQ(JFrame frame) {
        super(frame);
    }
    ...
    protected final DownloadObject[] getNeededImages() {
        ...
        for (CardPrinted c : CardDb.instance().getAllCards()) {
            ...
            if (!foundSetImage) {
                ...
                cList.add(new DownloadObject(fn, URLBase + SC2 + "/" + Base64Coder.encodeString(fn, true), base.getPath() + File.separator + SC3));
            }
        }
