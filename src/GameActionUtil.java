...
public class GameActionUtil
{
			...
			Ability ability2 = new Ability(card, "0")
			{
				public void resolve()
				{
					PlayerLife life = AllZone.GameAction
							.getPlayerLife(controller);
					life.addLife(1);
				}
			}; // ability2

			ability2.setStackDescription(card.getName() + " - "
					+ c.getController() + " played a black spell, "
	...
	public static void playCard_Enchantress_Draw(Card c)
	{
		...
		PlayerZone play = AllZone
				.getZone(Constant.Zone.Play, c.getController());
	...
	public static void playCard_Sigil_of_the_Empty_Throne(Card c)
	{
		...
		final PlayerZone play = AllZone.getZone(Constant.Zone.Play, c
				.getController());
	...
	public static void playCard_Quirion_Dryad(Card c)
	{
					...
					CardUtil.getColors(c).contains(Constant.Color.Red)))
		{
			for (int i=0;i<list.size();i++)
			{
				...
				ability2.setStackDescription(card.getName() + " - "
						+ c.getController() + " played a white, blue, black or red spell,  "
						+ " gets a +1/+1 counter.");
				AllZone.Stack.add(ability2);
			}
		}
	...
	public static void playCard_Mold_Adder(Card c)
	{
					...
					CardUtil.getColors(c).contains(Constant.Color.Black) ))
		{
			for (int i=0;i<list.size();i++)
			{
				...
				ability2.setStackDescription(card.getName() + " - "
						+ c.getController() + " played a blue or black spell,  "
						+ card.getName() + " gets a +1/+1 counter.");
				AllZone.Stack.add(ability2);
		...
		if (list.size() > 0){
			if (CardUtil.getColors(c).contains(Constant.Color.Blue))
			{
					for (int i=0;i<list.size();i++)
					{
						...
						ability2.setStackDescription(card.getName() + " - "
								+ c.getController() + " played a blue spell, put a 1/1 blue Bird token with flying into play.");
						AllZone.Stack.add(ability2);
					}
				}//if
			}
			...
			if (CardUtil.getColors(c).contains(Constant.Color.Green))
			{
					for (int i=0;i<list.size();i++)
					{
						...
						ability.setStackDescription(card.getName() + " - "
								+ c.getController() + " played a green spell, put a 2/2 green Wolf token into play.");
		...
		if (list.size() > 0)
		{
			...
			final Ability ability = new Ability(list.get(0), "0")
		    {
		        public void chooseTargetAI()
		        {
		          ...
		          if(cards.isEmpty() || AllZone.Human_Life.getLife() < 5)
		            setTargetPlayer(Constant.Player.Human);
		          else
		            setTargetCard(cards.get(0));
		        }
		        public void resolve()
		        {
	...
	public static void upkeep_removeDealtDamageToOppThisTurn()
	{
		...
		{
			Card c = list.get(i);
			if (c.getDealtCombatDmgToOppThisTurn())
				c.setDealtCombatDmgToOppThisTurn(false);
			if (c.getDealtDmgToOppThisTurn())
	...
	public static void removeAttackedBlockedThisTurn()
	{
		...
		for (int i = 0; i < list.size(); i++)
		{
			...
			if (c.getCreatureBlockedThisTurn())
				c.setCreatureBlockedThisTurn(false);
			
			if(c.getCreatureGotBlockedThisTurn())
	...
	public static void executeLandfallEffects(Card c)
	{
		if (c.getName().equals("Rampaging Baloths"))
			landfall_Rampaging_Baloths(c);
		else if (c.getName().equals("Emeria Angel"))
			landfall_Emeria_Angel(c);
		else if (c.getName().equals("Ob Nixilis, the Fallen"))
			landfall_Ob_Nixilis(c);
	...
	public static void executeLifeLinkEffects(Card c)
	{
		...
		ability2.setStackDescription(c.getName() + " (Lifelink) - " + player
				+ " gains " + power + " life.");
		AllZone.Stack.add(ability2);
	}
	...
	public static void executeVampiricEffects(Card c)
	{
		...
		for (int i = 0; i < a.size(); i++) {
			if (AllZone.GameAction.isCardInPlay(c) && a.get(i).toString().startsWith("Whenever a creature dealt damage by this card this turn is put into a graveyard, put"))
			{
				...
				Ability ability2 = new Ability(c, "0")
				{
					public void resolve()
					{
						if (AllZone.GameAction.isCardInPlay(thisCard))
	...
	public static void executePlayerCombatDamageEffects(Card c)
	{
		...
		else if (c.getName().equals("Rootwater Thief"))
	...
	private static void playerCombatDamage_Spawnwrithe(Card c)
	{
		...
		Ability ability2 = new Ability(c, "0")
		{
			public void resolve()
			{
				...
				card.setToken(true);
				card
						.setText("Whenever Spawnwrithe deals combat damage to a player, put a token into play that's a copy of Spawnwrithe.");

				card.addType("Creature");
				card.addType("Elemental");
				card.addIntrinsicKeyword("Trample");
				card.setBaseAttack(2);
				card.setBaseDefense(2);

				PlayerZone play = AllZone.getZone(Constant.Zone.Play, card
						.getController());
				play.add(card);
			}
		};// ability2
	...
	private static void playerCombatDamage_Goblin_Lackey(Card c)
	{
		if (c.getNetAttack() > 0)
		{
				...
				public void resolve()
				{
					...
					goblins = goblins.filter(new CardListFilter()
					{
						...
						public boolean addCard(Card c) {
							return (c.getType().contains("Goblin") || c.getKeyword().contains("Changeling")) && c.isPermanent();
						}
					...
					if(goblins.size() > 0)
					{
						...
						{
							Object o = AllZone.Display.getChoiceOptional("Select a Goblin to put into play", goblins.toArray());
			...
			ability2.setStackDescription(c.getName() + " - " + c.getController() + " puts a goblin into play from his or her hand.");
			AllZone.Stack.add(ability2);
		}
	}
			...
			Ability ability2 = new Ability(c, "0")
			{
				public void resolve()
				{
					...
					goblins = goblins.filter(new CardListFilter()
					{
						...
						public boolean addCard(Card c) {
							return (c.getType().contains("Goblin") || c.getKeyword().contains("Changeling")) && c.isCreature();
						}
	...
	@SuppressWarnings("unused") // upkeep_CheckEmptyDeck_Lose
	private static void upkeep_CheckEmptyDeck_Lose()
	{
		...
		System.out.println("libraryZone.size: " + libraryZone.size()
				+ " phase: " + AllZone.Phase.getPhase() + "Turn: "
				+ AllZone.Phase.getTurn());
		if (libraryZone.size() == 0
				&& AllZone.Phase.getPhase().equals(Constant.Phase.Untap)
	...
	{
		PlayerZone AIHand = AllZone.getZone(Constant.Zone.Hand,
				Constant.Player.Computer);
		PlayerZone AIPlay = AllZone.getZone(Constant.Zone.Play,
				Constant.Player.Computer);
		...
		for (int i = 0; i < AIHand.size(); i++)
		{
			if (AIHand.get(i).getType().contains("Creature")
					&& CardUtil.getConvertedManaCost(AIHand.get(i)
							.getManaCost()) <= 3)
				creatures.add(AIHand.get(i));
		}
	...
	private static void upkeep_Land_Tax()
	{
		...
		PlayerZone oppPlayZone = AllZone.getZone(Constant.Zone.Play,
		...
		if (self.size() < opp.size())
		{
			...
			for (int i = 0; i < list.size(); i++)
			{
				...
				ability
						.setStackDescription("Land Tax - search library for up to three basic land cards and put them into your hand");
				AllZone.Stack.add(ability);
	...
	private static void upkeep_Squee()
	{
		...
		for (int i = 0; i < list.size(); i++)
		{
			Ability ability = new Ability(list.get(i), "0")
			{
				...
				public void resolve()
				{
					...
					if (player.equals("Human"))
					{
						...
						Object o = AllZone.Display
								.getChoiceOptional(
										"Return Squee from your graveyard to your hand?",
										choices);
						if (o.equals("Yes"))
						{
			...
			ability
					.setStackDescription("Squee gets returned from graveyard to hand.");
			AllZone.Stack.add(ability);
		} // if creatures > 0
	...
	private static void upkeep_AEther_Vial()
	{
		...
		if (list.size() > 0)
		{
			for (int i = 0; i < list.size(); i++)
			{
				...
				Ability ability = new Ability(list.get(i), "")
				{
					public void resolve() {
						...
						if (player.equals(Constant.Player.Human)){
							q = AllZone.Display.getChoiceOptional("Put a counter on AEther Vial? (" + thisCard.getCounters(Counters.CHARGE) + ")", choices);
							if (q == null || q.equals("No"))
								return;
							if (q.equals("Yes"))
							{
				...
				ability.setStackDescription(list.get(i).getName() + " (" + list.get(i).getCounters(Counters.CHARGE) + " counters) - Put a charge counter on AEther Vial?");
				AllZone.Stack.add(ability);
			}//for
		}
		...
		if (list.size() > 0)
		{
			for (int i = 0; i < list.size(); i++)
			{
				...
				if (lands.size() >= 5)
				{
					...
					ability
							.setStackDescription("Scute Mob - put four +1/+1 counters on Scute Mob.");
					AllZone.Stack.add(ability);
				}
			} // for
		} // if creatures > 0
	...
	private static void upkeep_Heartmender()
	{
		...
		if (list.size() > 0)
		{
			for (int i = 0; i < list.size(); i++)
			{
				...
				ability
						.setStackDescription("Heartmender - Remove a -1/-1 counter from each creature you control.");
				AllZone.Stack.add(ability);
			} // for
		} // if creatures > 0
	}//heartmender
	...
	private static void upkeep_Ratcatcher()
	{
		...
		if (creatures.size() > 0 && list.size() > 0)
		{
			for (int i = 0; i < list.size(); i++)
			{
						...
						if (rats.size() > 0)
						{
							if (player.equals("Human"))
							{
								Object o = AllZone.Display.getChoiceOptional(
										"Pick a Rat to put into your hand", rats
												.toArray());
								if (o != null)
								{
									Card card = (Card) o;
				...
				ability
						.setStackDescription("Ratcatcher - search library for a rat and put into your hand");
				AllZone.Stack.add(ability);
			} // for
		} // if creatures > 0
	}
	...
	private static void upkeep_Nath()
	{
		...
		for (int i = 0; i < list.size(); i++)
		{
			...
			ability.setStackDescription("Nath of the Gilt-Leaf - " + opponent
		...
		for (int i = 0; i < list.size(); i++)
		{
			...
			ability = new Ability(list.get(i), "0")
			{
				public void resolve()
				{
					...
					{
						Object o = AllZone.Display
								.getChoiceOptional("Select Card to discard",
										cardsInHand.toArray());
						Card c = (Card) o;
			...
			ability
					.setStackDescription("Cunning Lethemancer - Everyone discards a card.");
			AllZone.Stack.add(ability);
		}
	}
	/*
	...
	*/
	...
	private static void upkeep_Sensation_Gorger()
	{
		...
		for (int i = 0; i < list.size(); i++)
		{
      	  	...
      	  	if (library.size() <= 0)
			{
      	  	...
      	  	{
				String[] choices = { "Yes", "No" };
				Object o = AllZone.Display.getChoiceOptional("Use " + list.get(i).getName() +  "'s ability?", choices);
			...
			if (creatureType.contains("Goblin")
					|| creatureType.contains("Shaman") || library.get(0).getKeyword().contains("Changeling"))
			{
				Card[] c = hand.getCards();
		        for(int q = 0; q < c.length; q++)
		            AllZone.GameAction.discard(c[q]);
	...
	private static void upkeep_Winnower_Patrol()
	{
		...
		for (int i = 0; i < list.size(); i++)
		{
			...
			System.out.println("CardName: " + list.get(i) + " id: "
					+ list.get(i).getUniqueNumber());
			...
			final Ability ability = new Ability(list.get(i), "0")
			{
				public void resolve()
				{
					...
					String creatureType = library.get(cardIndex).getType()
							.toString();
					

					if (creatureType.contains("Elf")
							|| creatureType.contains("Warrior") || library.get(cardIndex).getKeyword().contains("Changeling"))
					{
			...
			if (creatureType.contains("Elf")
					|| creatureType.contains("Warrior"))
				ability.setStackDescription("Winnower Patrol - " + player
						+ " reveals: " + cardName
						+ ", and Winnower Patrol gets +1/+1.");
			else
				ability.setStackDescription("Winnower Patrol - " + player
						+ " reveals top card: " + cardName + ".");
	...
	private static void upkeep_Nightshade_Schemers()
	{
		...
		for (int i = 0; i < list.size(); i++)
		{
			...
			if (creatureType.contains("Faerie")
				...
				ability.setStackDescription("Nightshade Schemers - " + player
						+ " reveals: " + cardName + ", and " + opponent
						+ " loses 2 life.");
			else
				ability.setStackDescription("Nightshade Schemers - " + player
						+ " reveals top card: " + cardName + ".");
		...
		for (int i = 0; i < list.size(); i++)
		{
			...
			ability = new Ability(list.get(i), "0")
			{
				public void resolve()
				{
					...
					if (creatureType.contains("Giant") || creatureType.contains("Wizard") || library.get(0).getKeyword().contains("Changeling"))
					{
			...
			if (creatureType.contains("Giant")
					|| creatureType.contains("Wizard"))
				ability.setStackDescription("Wandering Graybeard - " + player
						+ " reveals: " + cardName + ", and gains 4 life.");
			else
				ability.setStackDescription("Wandering Graybeard - " + player
						+ " reveals top card: " + cardName + ".");
	...
	private static void upkeep_Wolf_Skull_Shaman()
	{
		...
		for (int i = 0; i < list.size(); i++)
		{
			...
			if (creatureType.contains("Elf") || creatureType.contains("Shaman"))
				ability.setStackDescription("Wolf-Skull Shaman - " + player
						+ " reveals: " + cardName
						+ ", and puts 2/2 Wolf into play.");
			else
				ability.setStackDescription("Wolf-Skull Shaman - " + player
						+ " reveals top card: " + cardName + ".");
	...
	private static void upkeep_Leaf_Crowned_Elder()
	{
		...
		for (int i = 0; i < list.size(); i++)
		{
			...
			if (creatureType.contains("Treefolk")
					|| creatureType.contains("Shaman"))
				ability.setStackDescription("Leaf-Crowned Elder - " + player
						+ " puts into play: " + cardName + ".");
			else
				ability.setStackDescription("Leaf-Crowned Elder - " + player
						+ " reveals top card: " + cardName + ".");
	...
	private static void upkeep_Dark_Confidant()
	{
		...
		for (int i = 0; i < list.size(); i++)
		{
			...
			// System.out.println("top of deck: " + library.get(i).getName());
			final int convertedManaCost = CardUtil.getConvertedManaCost(library
					.get(i).getManaCost());
			String cardName = library.get(i).getName();
			...
			ability = new Ability(list.get(i), "0")
			{
				public void resolve()
				{
					...
					PlayerZone hand = AllZone.getZone(Constant.Zone.Hand,
							player);
					AllZone.GameAction.getPlayerLife(player).subtractLife(
							convertedManaCost);
			...
			ability.setStackDescription("Dark Confidant - " + player
					+ " loses " + convertedManaCost
			...
			{
			{
				Ability ability = new Ability(list.get(i), "0")
				{
					public void resolve()
					{
						...
						{
							Object o = AllZone.Display.getChoiceOptional(
									"Pick a creature to put into play", creatures
											.toArray());
							if (o != null)
							{
								Card card = (Card) o;
			...
			for (int i=0;i<list.size();i++)
			{
				Ability ability = new Ability(list.get(0), "0")
				{
					public void resolve()
					{
						...
						{
							Object o = AllZone.Display.getChoiceOptional(
									"Pick a creature to put into play", creatures.toArray());
							if (o != null)
							{
								Card card = (Card) o;
								graveyard.remove(card);
				...
				Ability ability = new Ability(list.get(0), "0")
				{
					public void resolve()
					{
						...
						if (creatures.size() >= 4)
						{
							...
							{
								Object o = AllZone.Display.getChoiceOptional(
										"Pick a creature to return to hand", creatures.toArray());
								if (o != null)
								{
									Card card = (Card) o;
									graveyard.remove(card);
	...
	private static void upkeep_Reya()
	{
		...
		if (creatures.size() > 0 && list.size() > 0)
		{
			...
			ability
					.setStackDescription("Reya returns creature from graveyard back to play");
			AllZone.Stack.add(ability);
		} // if creatures > 0
	} // reya
		...
		if (creatures.size() > 0 && list.size() > 0)
		{
			Ability ability = new Ability(list.get(0), "0")
			{
				public void resolve()
				{
					...
					{
						Object o = AllZone.Display.getChoiceOptional(
								"Pick a goblin to put into your hand",
								creatures.toArray());
						if (o != null)
						{
							Card card = (Card) o;
			...
			ability
					.setStackDescription("Wort returns creature from graveyard to "
							+ player + "'s hand");
			AllZone.Stack.add(ability);
		} // if creatures > 0
	} // Wort
	...
	private static void upkeep_Spore_Counters()
	{
		...
		{
			Card c = list.get(i);
			if (c.getName().equals("Deathspore Thallid")
					|| c.getName().equals("Elvish Farmer")
					|| c.getName().equals("Feral Thallid")
					|| c.getName().equals("Mycologist")
					|| c.getName().equals("Pallid Mycoderm")
					|| c.getName().equals("Psychotrope Thallid")
					|| c.getName().equals("Savage Thallid")
					|| c.getName().equals("Thallid")
					|| c.getName().equals("Thallid Devourer")
					|| c.getName().equals("Thallid Germinator")
					|| c.getName().equals("Thallid Shell-Dweller")
					|| c.getName().equals("Thorn Thallid")
					|| c.getName().equals("Utopia Mycon")
					|| c.getName().equals("Vitaspore Thallid"))
			{
				c.addCounter(Counters.SPORE, 1);
			}
		}
		...
		if (0 < list.size())
		{
			...
			for (int i = 0; i < list.size(); i++)
			{
				...
				Ability ability = new Ability(list.get(0), "0")
				{
					public void resolve()
					{
						PlayerZone library = AllZone.getZone(
								Constant.Zone.Library, card.getController());
						PlayerZone play = AllZone.getZone(Constant.Zone.Play,
								card.getController());
					...
					public Card getCreatureFromLibrary()
					{
						...
						if (Constant.Player.Computer.equals(card
								.getController()))
						{
							return CardFactoryUtil
									.AI_getBestCreature(creatureList);
						} else
						{
							Object o = AllZone.Display.getChoiceOptional(
									"Choose a creature card", creatureList
											.toArray());
							if (o != null)
							{
				...
				ability
						.setStackDescription("Defense of the Heart - "
								+ player
	...
	private static void upkeep_Convalescence()
	{
		...
		for (int i = 0; i < list.size(); i++)
		{
			...
			ability.setStackDescription("Convalescence - " + player
					+ " gain 1 life");
	...
	private static void upkeep_BlackVice()
	{
		...
		final String player = AllZone.Phase.getActivePlayer();
		final int playerHandSize = AllZone.getZone(Constant.Zone.Hand, player)
				.size();
		...
		PlayerZone opponentPlayZone = AllZone.getZone(Constant.Zone.Play,
				opponent);
		...
		if (0 < blackVice.size())
		{
			for (int i = 0; i < blackVice.size(); i++)
			{
				...
				ability.setStackDescription("Black Vise deals " + damage
						+ " to " + player);
				AllZone.Stack.add(ability);
			}
		}// if
	...
	private static void upkeep_Klass()
	{
		...
		if (0 < list.size() && 10 <= elf.size())
		{
			...
			ability.setStackDescription("Klaas, Elf Friend - " + player
					+ " wins the game");
			AllZone.Stack.add(ability);
		}// if
	}// upkeep_Klass
	...
	private static void upkeep_Felidar_Sovereign()
	{
		...
		if (0 < list.size() && plife.getLife() >= 40)
		{
			...
			ability.setStackDescription("Felidar Sovereign - " + player
					+ " wins the game");
			AllZone.Stack.add(ability);
		}// if
	}// upkeep_Felidar_Sovereign
	...
	private static void upkeep_Battle_of_Wits()
	{
		...
		if (0 < list.size() && 200 <= libraryZone.size())
		{
			...
			ability.setStackDescription("Battle of Wits - " + player
					+ " wins the game");
			AllZone.Stack.add(ability);
		}// if
	}// upkeep_Battle_of_Wits
	...
	private static void upkeep_Greener_Pastures()
	{
		...
		if ((self.size() == opp.size()) || opp.size() > self.size())
			return;
		else //active player has more lands
		{
			String mostLandsPlayer = "";
			if (self.size() > opp.size())
				mostLandsPlayer = player;
			...
			for (int i = 0; i < list.size(); i++)
			{
				ability = new Ability(list.get(i), "0")
				{
					public void resolve()
					{
						...
						c.setOwner(mostLands);
						c.setController(mostLands);
				...
				ability.setStackDescription("Greener Pastures - " + mostLands + " puts a 1/1 green Saproling token into play.");
		...
		for (int i = 0; i < list.size(); i++)
		{
			ability = new Ability(list.get(i), "0")
			{
				public void resolve()
				{
					...
					c.setName("Faerie Rogue");
					c.setImageName("B 1 1 Faerie Rogue");
					c.setManaCost("B");
					c.setToken(true);
			...
			ability.setStackDescription("Bitterblossom - deals 1 damage to "
					+ player + " and put a 1/1 token into play.");
		...
		for (int i = 0; i < list.size(); i++)
		{
			...
			ability = new Ability(list.get(i), "0")
			{
				public void resolve()
				{
					...
					if (player.equals(Constant.Player.Human))
					{
						Object o = AllZone.Display.getChoiceOptional("Select creature to sacrifice", creats.toArray());
						Card sac = (Card)o;
						if (sac == null){
							creats.shuffle();
							sac = creats.get(0);
			...
			ability.setStackDescription("Eldrazi Monument - "
	...
	private static void upkeep_Mycoloth()
	{
		...
		for (int i = 0; i < list.size(); i++)
		{
			...
			ability.setStackDescription("Mycoloth - "
					+ player + " puts a 1/1 green Saproling into play for each +1/+1 counter on Mycoloth.");
	...
	private static void upkeep_Bringer_of_the_Green_Dawn()
	{
		...
		for (int i = 0; i < list.size(); i++)
		{
			...
			ability.setStackDescription("Bringer of the Green Dawn - "
					+ player + " puts a 3/3 Green Beast token creature into play.");
	...
	private static void upkeep_Bringer_of_the_White_Dawn()
	{
		...
		if (artifacts.size() > 0 && list.size() > 0)
		{
			...
			for (int i = 0; i < list.size(); i++)
			{
				...
				ability.setStackDescription("Bringer of the White Dawn - "
						+ player + " returns an artifact to play.");
	...
	private static void upkeep_Serendib_Efreet()
	{
		...
		for (int i = 0; i < list.size(); i++)
		{
			...
			ability.setStackDescription("Serendib Efreet - deals 1 damage to "
					+ player);
	...
	private static void upkeep_Nettletooth_Djinn()
	{
		...
		for (int i = 0; i < list.size(); i++)
		{
			...
			ability
					.setStackDescription("Nettletooth Djinn - deals 1 damage to "
							+ player);
	...
	private static void upkeep_Seizan_Perverter_of_Truth()
	{
		...
		ability.setStackDescription("Seizan, Perverter of Truth - " + player
				+ " loses 2 life and draws 2 cards");
	...
	private static void upkeep_Juzam_Djinn()
	{
		...
		for (int i = 0; i < list.size(); i++)
		{
			...
			ability.setStackDescription("Juzam Djinn - deals 1 damage to "
					+ player);
	...
	private static void upkeep_Fledgling_Djinn()
	{
		...
		for (int i = 0; i < list.size(); i++)
		{
			...
			ability.setStackDescription("Fledgling Djinn - deals 1 damage to "
					+ player);
	...
	public static Command Conspiracy = new Command()
	{
		...
		{			
			...
			for (int i=0;i < cl.size(); i++)
			{
				...
				zone[0] = AllZone.getZone(Constant.Zone.Hand, player);
				zone[1] = AllZone.getZone(Constant.Zone.Library, player);
				zone[2] = AllZone.getZone(Constant.Zone.Graveyard, player);
				zone[3] = AllZone.getZone(Constant.Zone.Play, player);
				...
				for (int outer = 0; outer < zone.length; outer++)
				{
					...
					for (int j = 0; j < creature.size(); j++)
					{
						...
						if (c.getPrevType().size()== 0)
							c.setPrevType(c.getType());
						c.setType(new ArrayList<String>());
						c.addType("Creature");
						if (art)
	    					c.addType("Artifact");
	    				if (ench)
	    					c.addType("Enchantment");
	...
	public static Command Engineered_Plague = new Command()
	{
		...
		public void execute()
		{
			...
			for (int outer = 0; outer < cards.size(); outer++)
			{
				...
				{
					c = creature.get(i);
					if(c.getType().contains(card.getChosenType()) || c.getKeyword().contains("Changeling"))
					{
						c.addSemiPermanentAttackBoost(-1);
						c.addSemiPermanentDefenseBoost(-1);
	...
	public static Command That_Which_Was_Taken = new Command()
	{
		...
		public void execute()
		{
			...
			if (zone.length>0)
			{
				...
				{
					c = cards.get(i);
					if (!c.getKeyword().contains(keyword) && c.getCounters(Counters.DIVINITY) > 0)
					{
						c.addExtrinsicKeyword(keyword);
						gloriousAnthemList.add(c);
					}
	...
	public static Command Reach_of_Branches = new Command()
	{
		...
		public void execute()
		{
			...
			final String player = AllZone.Phase.getActivePlayer();
			final PlayerZone grave = AllZone.getZone(Constant.Zone.Graveyard,
					player);
			...
			if (0 < nCard.size() && newForest(oldForest, newForest))
			{
				...
				ability
						.setStackDescription("Reach of Branches - return card to "
								+ player + "'s hand");
				AllZone.Stack.add(ability);
			}// if
	...
	public static Command Baru = new Command()
	{
		...
		public void execute()
		{
			...
			for (int outer = 0; outer < current.size(); outer++)
			{
				...
				SpellAbility ability = new Ability(new Card(), "0")
				{
					public void resolve()
					{
						...
						for (int i = 0; i < all.size(); i++)
						{
							...
							all.get(i).addExtrinsicKeyword("Trample");
				...
				ability
						.setStackDescription("Baru, Fist of Krosa - creatures get +1/+1 until end of turn.");
	...
	public static Command Wirewood_Hivemaster = new Command()
	{
		...
		public void execute()
		{
			...
			current = current.filter(new CardListFilter()
			{
				public boolean addCard(Card c)
				{
					return !c.isToken() && (c.getType().contains("Elf") || c.getKeyword().contains("Changeling") );
				}
			...
			for (int outer = 0; outer < hivemasterList.size(); outer++)
			{
				...
				final String player = hivemasterList.get(outer).getController();
				SpellAbility ability = new Ability(new Card(), "0")
				{
				...
				ability.setStackDescription(hivemasterList.get(outer).getName()
						+ " - " + hivemasterList.get(outer).getController()
						+ " puts " + n[0] + " insect tokens into play.");
	...
	public static Command Angelic_Chorus = new Command()
	{
		...
		public void execute()
		{
			...
			// List of all cards
			CardList playerCards = new CardList(playZone.getCards());
	...
	public static Command Dauntless_Dourbark = new Command()
	{
		...
		private boolean hasTreefolk(Card c)
		{
			...
			tree = tree.filter(new CardListFilter()
			{
				public boolean addCard(Card c)
				{
					return (c.getKeyword().contains("Changeling") || c.getType().contains("Treefolk")) && !c.getName().equals("Dauntless Dourbark");
				}
			});
			if (tree.size()>0)
				return true;
		...
		private int countTreeForests(Card c)
		{
			...
			list = list.filter(new CardListFilter()
			{
				public boolean addCard(Card c)
				{
					return c.getType().contains("Treefolk") || c.getKeyword().contains("Changeling") || c.getType().contains("Forest");
				}
			});
	...
	public static Command Liu_Bei = new Command()
	{
		...
		private boolean getsBonus(Card c)
		{
			...
			{
				public boolean addCard(Card c) {
					return c.getName().equals("Guan Yu, Sainted Warrior") || c.getName().equals("Zhang Fei, Fierce Warrior");
				}
		...
		public void execute()
		{
			...
			for (int outer = 0; outer < zone.length; outer++)
			{
				...
				for (int i = 0; i < perms.size(); i++)
				{
					c = perms.get(i);
					if (c.isPermanent()
							&& !c.getName().equals("Privileged Position") 
							&& !c.getExtrinsicKeyword().contains("This card can't be the target of spells or abilities your opponents control."))
					{
		...
		public void execute()
		{
			...
			for (int outer = 0; outer < zone.length; outer++)
			{
				...
				for (int i = 0; i < creature.size(); i++)
				{
					c = creature.get(i);
					if (c.isCreature()
							&& !c.getName().equals("Elvish Archdruid"))
					{
						c.addSemiPermanentAttackBoost(1);
						c.addSemiPermanentDefenseBoost(1);
						gloriousAnthemList.add(c);
		...
		public void execute()
		{
			...
			for (int outer = 0; outer < zone.length; outer++)
			{
				...
				for (int i = 0; i < creature.size(); i++)
				{
					c = creature.get(i);
					if (c.isCreature()
							&& !c.getName().equals("Elvish Champion"))
					{
						c.addSemiPermanentAttackBoost(1);
						c.addSemiPermanentDefenseBoost(1);
	...
	public static Command Elvish_Champion_Other = new Command()
	{
		...
		public void execute()
		{
			...
			for (int i = 0; i < creature.size(); i++)
			{
				...
				if (!c.getExtrinsicKeyword().contains("Forestwalk") && otherLords > 0)
					c.addExtrinsicKeyword("Forestwalk");
	...
	public static Command Goblin_King_Other = new Command()
	{
		...
		public void execute()
		{
			...
			for (int i = 0; i < creature.size(); i++)
			{
				...
				c.setOtherAttackBoost(otherLords);
				c.setOtherDefenseBoost(otherLords);
				if (!c.getExtrinsicKeyword().contains("Mountainwalk") && otherLords > 0)
					c.addExtrinsicKeyword("Mountainwalk");
		...
		public void execute()
		{
			...
			for (int outer = 0; outer < zone.length; outer++)
			{
				...
				for (int i = 0; i < creature.size(); i++)
				{
					c = creature.get(i);
					if (c.isCreature()
							&& !c.getName().equals("Lord of Atlantis"))
					{
						c.addSemiPermanentAttackBoost(1);
						c.addSemiPermanentDefenseBoost(1);
	...
	public static Command Lord_of_Atlantis_Other = new Command()
	{
		...
		public void execute()
		{
			...
			for (int i = 0; i < creature.size(); i++)
			{
				...
				c.setOtherAttackBoost(otherLords);
				c.setOtherDefenseBoost(otherLords);
				if (!c.getExtrinsicKeyword().contains("Islandwalk") && otherLords > 0)
					c.addExtrinsicKeyword("Islandwalk");
		...
		public void execute()
		{
			...
			for (int outer = 0; outer < zone.length; outer++)
			{
				...
				for (int i = 0; i < creature.size(); i++)
				{
					c = creature.get(i);
					if (c.isCreature()
							&& !c.getName().equals("Field Marshal"))
					{
						c.addSemiPermanentAttackBoost(1);
						c.addSemiPermanentDefenseBoost(1);
	...
	public static Command Field_Marshal_Other = new Command()
	{
		...
		public void execute()
		{
			...
			for (int i = 0; i < creature.size(); i++)
			{
				...
				if (!c.getExtrinsicKeyword().contains("First Strike") && otherMarshals > 0)
					c.addExtrinsicKeyword("First Strike");
	...
	public static Command Aven_Brigadier_Soldier_Pump = new Command()
	{
		...
		public void execute()
		{
			...
			for (int outer = 0; outer < zone.length; outer++)
			{
				...
				{
					c = creature.get(i);
					if (c.isCreature() && !c.getName().equals("Aven Brigadier"))
					{
						c.addSemiPermanentAttackBoost(1);
						c.addSemiPermanentDefenseBoost(1);
						gloriousAnthemList.add(c);
		...
		public void execute()
		{
			...
			for (int outer = 0; outer < zone.length; outer++)
			{
				...
				for (int i = 0; i < creature.size(); i++)
				{
					c = creature.get(i);
					if (c.isCreature()
							&& !c.getName().equals("Scion of Oona"))
					{
						c.addSemiPermanentAttackBoost(1);
						c.addSemiPermanentDefenseBoost(1);
						c.addExtrinsicKeyword("Shroud");
	...
	public static Command Master_of_Etherium = new Command()
	{
		...
		@SuppressWarnings("unused") // gloriousAnthemList
		CardList gloriousAnthemList = new CardList();
		
		@SuppressWarnings("unused") // otherMasters
		int otherMasters=0;
	...
	public static Command Master_of_Etherium_Pump = new Command()
	{
		...
		public void execute()
		{
			...
			for (int outer = 0; outer < zone.length; outer++)
			{
				...
				for (int i = 0; i < creature.size(); i++)
				{
					...
					c = creature.get(i);
					if (c.isCreature()
							&& !c.getName().equals("Master of Etherium"))
					{
						c.addSemiPermanentAttackBoost(1);
						c.addSemiPermanentDefenseBoost(1);
						gloriousAnthemList.add(c);
		...
		@SuppressWarnings("unused") // countArtifacts
		private int countArtifacts(Card c)
		{
			PlayerZone play = AllZone.getZone(Constant.Zone.Play, c.getController());
			CardList artifacts = new CardList(play.getCards());
		...
		@SuppressWarnings("unused") // countOtherMasters
		private int countOtherMasters(Card c)
		{
			PlayerZone play = AllZone.getZone(Constant.Zone.Play, c
					.getController());
	...
	public static Command Loxodon_Punisher = new Command()
	{
		...
		{
			CardList equipment = new CardList(c.getEquippedBy().toArray());
	...
	public static Command Rakdos_Pit_Dragon = new Command()
	{
		...
		public void execute()
		{
			...
			{
				Card c = list.get(i);
				if (hellbent(c.getController()) && !c.getIntrinsicKeyword().contains("Double Strike"))
					c.addIntrinsicKeyword("Double Strike");
				else if (!hellbent(c.getController()) && c.getIntrinsicKeyword().contains("Double Strike"))
		...
		public void execute()
		{
			...
			for (int i = 0; i < list.size(); i++)
			{
				...
				for (int j = 0; j < creatures.size(); j++)
				{
					...
					{
						if (c.getController()
								.equals(castleCard.getController())) // Only
						// apply
						// benefit
						// to
						// controlled
	...
	public static Command Valor = new Command()
	{
		...
		public void execute()
		{
			...
			if (isInGrave(Constant.Player.Computer))
				addFirstStrike(Constant.Player.Computer);

			if (isInGrave(Constant.Player.Human))
				addFirstStrike(Constant.Player.Human);
		}// execute()
		...
		boolean isInGrave(String player)
		{
			...
			if (!list.containsName("Valor") || lands.size() == 0)
				return false;
			else
				return true;
		}
		...
		{
			for (int i = 0; i < list.size(); i++)
				list.get(i).removeExtrinsicKeyword("First Strike");
		}
	...
	public static Command Anger = new Command()
	{
		...
		boolean isInGrave(String player)
		{
			...
			if (!list.containsName("Anger") || lands.size() == 0)
				return false;
			else
				return true;
		}
	...
	public static Command Wonder = new Command()
	{
		...
		boolean isInGrave(String player)
		{
			...
			if (!list.containsName("Wonder") || lands.size() == 0)
				return false;
			else
				return true;
		}
	...
	public static Command Brawn = new Command()
	{
		...
		public void execute()
		{
			...
			if (isInGrave(Constant.Player.Computer))
				addTrample(Constant.Player.Computer);

			if (isInGrave(Constant.Player.Human))
				addTrample(Constant.Player.Human);
		}// execute()
		...
		boolean isInGrave(String player)
		{
			...
			if (!list.containsName("Brawn") || lands.size() == 0)
				return false;
			else
				return true;
		}
		...
		public void execute()
		{
			...
			for (int outer = 0; outer < zone.length; outer++)
			{
				...
				for (int i = 0; i < creature.size(); i++)
				{
					c = creature.get(i);
					if (CardUtil.getColors(c).contains(Constant.Color.Green)
							&& !c.getName().equals("Meng Huo, Barbarian King"))
					{
						c.addSemiPermanentAttackBoost(1);
						c.addSemiPermanentDefenseBoost(1);
						gloriousAnthemList.add(c);
	...
	public static Command Eladamri = new Command()
	{
		...
		public void execute()
		{
			...
			for (int outer = 0; outer < zone.length; outer++)
			{
				...
				{
					c = creature.get(i);
					if (!c.getName().equals("Eladamri, Lord of Leaves"))
					{
						c.addExtrinsicKeyword("Shroud");
						c.addExtrinsicKeyword("Forestwalk");
						gloriousAnthemList.add(c);
	...
	public static Command Bad_Moon = new Command()
	{
		...
		public void execute()
		{
			...
			for (int outer = 0; outer < zone.length; outer++)
			{
				...
				{
					c = creature.get(i);
					if (CardUtil.getColors(c).contains(Constant.Color.Black) && !c.isFaceDown())
					{
						c.addSemiPermanentAttackBoost(1);
						c.addSemiPermanentDefenseBoost(1);
	...
	public static Command Crusade = new Command()
	{
		...
		public void execute()
		{
			...
			for (int outer = 0; outer < zone.length; outer++)
			{
				...
				{
					c = creature.get(i);
					if (CardUtil.getColors(c).contains(Constant.Color.White) && !c.isFaceDown())
					{
						c.addSemiPermanentAttackBoost(1);
						c.addSemiPermanentDefenseBoost(1);
						gloriousAnthemList.add(c);
	...
	public static Command Muraganda_Petroglyphs = new Command()
	{
		...
		public void execute()
		{
			...
			for (int outer = 0; outer < zone.length; outer++)
			{
				...
				{
					c = creature.get(i);
					if ((c.getText().equals("") && c.getKeyword().size() == 0) || c.isFaceDown())
					{
						c.addSemiPermanentAttackBoost(2);
						c.addSemiPermanentDefenseBoost(2);
	...
	private static PlayerZone[] getZone(String cardName)
	{
		...
		for (int i = 0; i < all.size(); i++)
			if (all.get(i).getName().equals(cardName) && !all.get(i).isFaceDown())
