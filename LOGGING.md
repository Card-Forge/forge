...
TODO: What are the key considerations here? Are there known limitations? Have we had bad experiences trying to transition from `System.*.print` statements to a logging framework before?
...
This is: datestamp, log level, thread name, classname with the package portions abbreviated, log message, and newline, with some formatting to help align elements more neatly across lines, improving scan-ability and readability. Here are some sample log lines demonstrating this pattern:

```
2025-06-13 15:51:13,082 [INFO ][ImageFetcher-07 ][f.u.SwingImageFetcher    ] Saved image to: /Users/forge-dev/Library/Caches/Forge/pics/cards/MKM/Extract a Confession.fullborder.jpg
2025-06-13 15:51:13,082 [TRACE][AWT-EventQueue-0][f.u.ImageFetcher         ] Removed: /Users/forge-dev/Library/Caches/Forge/pics/cards/MKM/Extract a Confession.full.jpg [queueSize: 8]
2025-06-13 15:51:13,148 [INFO ][ImageFetcher-06 ][f.u.SwingImageFetcher    ] Saved image to: /Users/forge-dev/Library/Caches/Forge/pics/cards/ARB/Glassdust Hulk.fullborder.jpg
2025-06-13 15:51:13,149 [TRACE][AWT-EventQueue-0][f.u.ImageFetcher         ] Removed: /Users/forge-dev/Library/Caches/Forge/pics/cards/ARB/Glassdust Hulk.full.jpg [queueSize: 7]
```
...
- Hanmac reports "I think right now, the logging is mostly done via Sentry, and then Sentry writes the log file." But the `sentry.properties` configuration files all reference a remote data source name (dsn), so that does not _appear_ to be relevant to the runtime development-centric logging under discussion here.
- The `log4j` and `logback` configuration files added to the repo earlier may be configuring logging for some dependencies whose logging conforms to the `slf4j` and `log4j` APIs. It appears that these could be satisfied and subsumed under an expanded `logback` configuration with a few minor changes.
