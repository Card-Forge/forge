...
public class SwingImageFetcher extends ImageFetcher {
    ...
    private static class SwingDownloadTask implements Runnable {
        ...
        private boolean doFetch(String urlToDownload) throws IOException {
            ...
            boolean isCardArtApiUrl = urlToDownload.startsWith(ForgeConstants.URL_PIC_SCRYFALL_DOWNLOAD)
                    || urlToDownload.startsWith(ForgeConstants.URL_SCRYFALL_CDN)
                    || urlToDownload.startsWith(ForgeConstants.URL_PIC_CARDFORGE_NET_API_DOWNLOAD);
            String newdespath = urlToDownload.contains(".fullborder.jpg") || isCardArtApiUrl ?
                    TextUtil.fastReplace(destPath, ".full.jpg", ".fullborder.jpg") : destPath;
            if (!newdespath.contains(".full") && !newdespath.contains(".artcrop") && isCardArtApiUrl && !destPath.startsWith(ForgeConstants.CACHE_TOKEN_PICS_DIR))
