...
public class SwingImageFetcher extends ImageFetcher {
    ...
    private static class SwingDownloadTask implements Runnable {
        ...
        private boolean doFetch(String urlToDownload) throws IOException {
            ...
            String newdespath = urlToDownload.contains(".fullborder.jpg") || urlToDownload.startsWith(getCardArtApiBaseUrl()) ?
                    TextUtil.fastReplace(destPath, ".full.jpg", ".fullborder.jpg") : destPath;
            if (!newdespath.contains(".full") && !newdespath.contains(".artcrop") && urlToDownload.startsWith(getCardArtApiBaseUrl()) && !destPath.startsWith(ForgeConstants.CACHE_TOKEN_PICS_DIR))
