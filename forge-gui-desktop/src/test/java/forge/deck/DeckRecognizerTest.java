...
public class DeckRecognizerTest extends CardMockTestCase {
    ...
    void testMatchDeckSectionNames() {
        ...
        String[] deckSectionEntriesFoundInMDExportFromTappedOut = new String[] { "## Sideboard (15)",
                "## Mainboard (86)" };
    ...
    void testMatchCardTypes() {
        ...
        String[] cardTypesEntriesFoundInMDExportFromTappedOut = new String[] { "### Instant (14)", "### Sorcery (9)",
                "### Artifact (14)", "### Creature (2)", "### Land (21)", };
        for (String entry : cardTypesEntriesFoundInMDExportFromTappedOut)
            assertTrue(DeckRecognizer.isCardType(entry), "Fail on " + entry);

        String[] cardTypesInDecFormat = new String[] { "//Lands", "//Artifacts", "//Enchantments", "//Instants",
                "//Sorceries", "//Planeswalkers", "//Creatures" };
    ...
    void testManaTokenMatch() {
        ...
        String[] expectedTokenText = new String[] { String.format("%s {U}", cname), String.format("%s {R}", cname),
                String.format("%s {W}", cname), String.format("%s {B}", cname), String.format("%s {C}", cname),
                String.format("%s {G}", cname), String.format("%s {W}{U}{B}{R}{G}", cname),
    ...
    void testManaTokensBiColors() {
        ...
        String[] mixedSymbols = new String[] { "{u} White", "{R}-black", "White {g}", "// {b} Blue", "(green|{r})" };
        String cname = CardMockTestCase.MOCKED_LOCALISED_STRING;
        String[] expectedTokenText = new String[] { String.format("%s/%s %s", cname, cname, "{WU}"),
                String.format("%s/%s %s", cname, cname, "{BR}"), String.format("%s/%s %s", cname, cname, "{GW}"),
                String.format("%s/%s %s", cname, cname, "{UB}"), String.format("%s/%s %s", cname, cname, "{RG}") };
    ...
    void testTokenBiColorSymbols() {
        ...
        String cname = CardMockTestCase.MOCKED_LOCALISED_STRING;
        String[] expectedTokenText = new String[] { String.format("%s/%s %s", cname, cname, "{WU}"),
                String.format("%s/%s %s", cname, cname, "{UB}"), String.format("%s/%s %s", cname, cname, "{BR}"),
                String.format("%s/%s %s", cname, cname, "{GW}"), String.format("%s/%s %s", cname, cname, "{RG}"),
                String.format("%s/%s %s", cname, cname, "{WB}"), String.format("%s/%s %s", cname, cname, "{UR}"),
                String.format("%s/%s %s", cname, cname, "{BG}"), String.format("%s/%s %s", cname, cname, "{RW}"),
    ...
    void testManaTokensRepeatedAreIgnored() {
        ...
        String[] manaTokens = new String[] { "{u} {u}", "{R}-{R}", "{w} {W}", "// {b} {B}", "({G}|{g})", };
        ...
        for (int i = 0; i < manaTokens.length; i++) {
            ...
            assertFalse(manaToken.getText()
                    .endsWith(String.format("%s %s", expectedManaSymbols[i], expectedManaSymbols[i])));
            assertFalse(manaToken.getText()
                    .endsWith(String.format("%s%s", expectedManaSymbols[i], expectedManaSymbols[i])));
            assertFalse(manaToken.getText()
                    .endsWith(String.format("%s/%s", expectedManaSymbols[i], expectedManaSymbols[i])));
    ...
    void testMultiColourOrColourlessManaTokensWillBeHandledSeparately() {
        ...
        String[] cmcTokens = new String[] { "Blue Colourless", "Red Multicolour", "Colorless White", "// Multicolour ",
                "(green|Colourless)" };
        String cname = CardMockTestCase.MOCKED_LOCALISED_STRING;
        String[] expectedTokenText = new String[] { String.format("%s {U} // %s {C}", cname, cname),
                String.format("%s {R} // %s {W}{U}{B}{R}{G}", cname, cname),
                String.format("%s {C} // %s {W}", cname, cname), String.format("%s {W}{U}{B}{R}{G}", cname),
    ...
    void testCornerCasesWithSpecialMulticolourAndColorlessTokens() {
        ...
        String cname = CardMockTestCase.MOCKED_LOCALISED_STRING;
        String[] expectedTokenText = new String[] { String.format("%s {C}", cname),
                String.format("%s {W}{U}{B}{R}{G}", cname), String.format("%s {C}", cname) };
    ...
    /*
     * ========================================= TOKEN/CARD PARSING: (Alternate) Set
     * Code ========================================
     */
    @Test
    ...
    /*
     * ======================================= TEST CONSTRAINTS: Edition Release
     * Date ======================================
     */
    @Test
    ...
    void testCardRequestUnderGameConstraints() {
        ...
        List<String> bannedList = Arrays.asList(StringUtils.split(
                ...
                ';'));
        List<String> allowedPioneerSets = Arrays.asList(StringUtils.split(
                "RTR,GTC,DGM,M14,THS,BNG,JOU,M15,KTK,FRF,DTK,ORI,BFZ,OGW,SOI,EMN,KLD,AER,AKH,HOU,XLN,RIX,DOM,M19,G18,GRN,RNA,WAR,M20,ELD,THB,IKO,M21,ZNR,KHM,STX,AFR,MID,VOW",
                ","));
    ...
    void testGameFormatRestrictionsAlsoWithRestrictedCardList() {
        ...
        List<String> restrictedCards = Arrays.asList(StringUtils.split(
    ...
    void testRecognizeDoubleSidedCards() {
        ...
        assertEquals(rightSideCard.getName(), leftSideRequest); // NOTE: this is not a blunder! Back side will result in
                                                                // front side name
        assertEquals(cardToken.getQuantity(), 1);
        assertTrue(cardToken.cardRequestHasNoCode());
    ...
    void testUnsupportedCardIsReturnedOnlyWhenNoOtherOptionExistForSectionMatching() {
        ...
        String[] cardList = new String[] { "1 Anowon, the Ruin Thief", // Commander Section but will go in MAIN +
                                                                       // placeholder
