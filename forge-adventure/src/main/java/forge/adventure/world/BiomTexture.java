...
public class BiomTexture {
    ...
    enum SmallPictures
    {
        ...
        public int value;
        SmallPictures(int i){value=i;}
    }
    public String name;
    public Pixmap emptyPixmap=new Pixmap(1,1, Pixmap.Format.RGB888);
    public BiomTexture(TextureAtlas tileSet,int tsize,String n) {
        name=n;
        Array<TextureAtlas.AtlasRegion> regions=tileSet.findRegions("TileSet");
        ...
        for (TextureAtlas.AtlasRegion region : regions)
        {
            ...
            for (int y = 0; y < 4; y++) {
                for (int x = 0; x < 3; x++) {
                    int px=(int) region.getRegionX() + (x * tsize);
                    int py=(int) region.getRegionY() + (y * tsize);
                    Pixmap subPixmap=new Pixmap(tsize,tsize, Pixmap.Format.RGBA8888);
                    subPixmap.drawPixmap(completePicture,0,0,px,py,tsize,tsize);
                    pics.add(subPixmap);
                }
            }
            for (int y = 0; y < 8; y++) {
                for (int x = 0; x < 6; x++) {
                    int px=  (int) region.getRegionX() + (x * tsize/2);
                    int py= (int) region.getRegionY() + (y * tsize / 2);
                    Pixmap subPixmap=new Pixmap(tsize/2,tsize/2, Pixmap.Format.RGBA8888);
                    subPixmap.drawPixmap(completePicture,0,0,px,py,tsize/ 2,tsize/ 2);
                    spics.add(subPixmap);
                }
            }
            images.add(pics);
    ...
    public Pixmap GetPixmapFor(int biomSubIndex, int neighbors, int heighestBiomID, Pixmap getPixmapFor)
    {

        int id=(neighbors*100)+heighestBiomID;
        if(biomSubIndex>=edgeImages.size||biomSubIndex<0)
        {
           return emptyPixmap;
        }
        ...
        int tileSize=getPixmapFor.getHeight();
        Pixmap subPixmap=new Pixmap(tileSize,tileSize, Pixmap.Format.RGBA8888);
        subPixmap.drawPixmap(getPixmapFor,0,0);
        switch(neighbors)
        {
