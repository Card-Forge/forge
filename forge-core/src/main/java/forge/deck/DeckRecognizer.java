...
public class DeckRecognizer {
    ...
    private boolean isBannedInFormat(PaperCard pc, DeckSection section) {
        ...
        if (section == DeckSection.Commander && this.deckFormat.isLegalCommander(pc)) {
            // A legal commander may still fail the main-deck rule — e.g. a Pauper Commander's
            // commander is an uncommon, which is intentionally not a legal ("commons only")
            // main-deck card. Everything else in the section (instants and sorceries land here
            // as potential signature spells whatever the format) keeps the main-deck check.
            return false;
        }
