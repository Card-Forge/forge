...
import forge.screens.match.input.InputSelectManyBase;
...
public class HumanCostDecision extends CostDecisionMakerBase {
    ...
    private PaymentDecision exileFromSame(CostExile cost, List<Card> list, int nNeeded, List<Player> payableZone) {
        ...
        final Player p = GuiChoose.oneOrNone(String.format("Exile from whose %s?", cost.getFrom()), payableZone);
        if (p == null) {
            return null;
        }
    ...
    private PaymentDecision exileFromStack(CostExile cost, SpellAbility sa, int nNeeded) {
        ...
        for (int i = 0; i < nNeeded; i++) {
            ...
            final String o = GuiChoose.oneOrNone("Exile from " + cost.getFrom(), descList);
    ...
    private PaymentDecision exileFromMiscZone(CostExile cost, SpellAbility sa, int nNeeded, List<Card> typeList) {
        ...
        List<Card> exiled = new ArrayList<Card>();
        for (int i = 0; i < nNeeded; i++) {
            final Card c = GuiChoose.oneOrNone("Exile from " + cost.getFrom(), typeList);
    ...
    private PaymentDecision putFromMiscZone(SpellAbility sa, int nNeeded, List<Card> typeList, ZoneType fromZone) {
        ...
        for (int i = 0; i < nNeeded; i++) {
            final Card c = GuiChoose.oneOrNone("Put from " + fromZone + " to library", typeList);
    ...
    private PaymentDecision putFromSame(List<Card> list, int nNeeded, List<Player> payableZone, ZoneType fromZone) {
        ...
        final Player p = GuiChoose.oneOrNone(String.format("Put cards from whose %s?", fromZone), payableZone);
        if (p == null) {
            return null;
        }
        ...
        List<Card> chosen = new ArrayList<>();
        for (int i = 0; i < nNeeded; i++) {
            final Card c = GuiChoose.oneOrNone("Put cards from " + fromZone + " to Library", typeList);
            if (c == null)
                return null;
            typeList.remove(c);
            chosen.add(c);
    ...
    public PaymentDecision visit(CostRemoveAnyCounter cost) {
        ...
        cost.setCounterType(GuiChoose.one(prompt, typeChoices));
