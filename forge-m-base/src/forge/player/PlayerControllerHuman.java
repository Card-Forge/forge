...
import forge.screens.match.input.InputAttack;
import forge.screens.match.input.InputBlock;
import forge.screens.match.input.InputConfirm;
import forge.screens.match.input.InputConfirmMulligan;
import forge.screens.match.input.InputPassPriority;
import forge.screens.match.input.InputProliferate;
import forge.screens.match.input.InputSelectCardsForConvoke;
import forge.screens.match.input.InputSelectCardsFromList;
import forge.screens.match.input.InputSelectEntitiesFromList;
...
public class PlayerControllerHuman extends PlayerController {
    ...
    public boolean isUiSetToSkipPhase(final Player turn, final PhaseType phase) {
        return !FControl.stopAtPhase(turn, phase);
    }
    ...
    public Map<Card, Integer> assignCombatDamage(Card attacker, List<Card> blockers, int damageDealt, GameEntity defender, boolean overrideOrder) {
        ...
        if (defender != null && assignDamageAsIfNotBlocked(attacker)) {
            ...
            if ((attacker.hasKeyword("Trample") && defender != null) || (blockers.size() > 1)) {
                map = FControl.getDamageToAssign(attacker, blockers, damageDealt, defender, overrideOrder);
            }
            else {
                map = new HashMap<Card, Integer>();
    ...
    public List<Card> chooseCardsForEffect(List<Card> sourceList, SpellAbility sa, String title, int min, int max, boolean isOptional) {
        ...
        FControl.highlightCard(sa.getHostCard());
    ...
    public List<Card> orderBlockers(Card attacker, List<Card> blockers) {
        FControl.highlightCard(attacker);
    ...
    public List<Card> orderAttackers(Card blocker, List<Card> attackers) {
        FControl.highlightCard(blocker);
